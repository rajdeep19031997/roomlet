...
class PublicAccountController extends Controller
{
    ...
    public function postSettings(SettingRequest $request, BaseHttpResponse $response)
    {
        ...
        $adhaarImage = NULL;
        $signatureImage = NULL;
        if($request->hasFile('adhaarImage')){
            $resizeFileName1 = time();
            $uploadPath1 = 'public/storage/banner';
            $uploadPathAc1 = 'public/storage/banner';
            $backImage1= rand(1111,9999).'image'.$resizeFileName1;
            $ext1=strtolower($request->adhaarImage->getClientOriginalExtension());
            $image_full_name1=$backImage1.'.'.$ext1;
            $upload_path1=$uploadPath1;
            $success1=$request->adhaarImage->move($upload_path1,$image_full_name1);
        ...
        if($request->hasFile('signatureImage')){
            ...
            $uploadPath2 = 'public/storage/banner';
            $uploadPathAc2 = 'public/storage/banner';
            $backImage2= rand(1111,9999).'image'.$resizeFileName2;
            $ext2=strtolower($request->signatureImage->getClientOriginalExtension());
            $image_full_name2=$backImage2.'.'.$ext2;
            $upload_path2=$uploadPath2;
            $success2=$request->signatureImage->move($upload_path2,$image_full_name2);
        ...
        DB::table('re_accounts')->where('id' , auth('account')->id())->update([
